...
/* Chuyển đổi giao diện web khi thực hiện thủ công */
if($trigger == "button"){
...
let outCurrentSpeed = speedTransform(network.outCurrentSpeed) //Tốc độ tải lên
let outMaxSpeed = speedTransform(network.outMaxSpeed) //Tốc độ tải lên tối đa
let download = bytesToSize(network.in) //Lưu lượng truy cập tải xuống
let upload = bytesToSize(network.out) //Lưu lượng truy cập tải lên
let inMaxSpeed = speedTransform(network.inMaxSpeed) //Tốc độ tải xuống tối đa
let inCurrentSpeed = speedTransform(network.inCurrentSpeed) //Tốc độ tải về
