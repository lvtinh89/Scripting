...
function loadCarrierNames() {
    //Logic sắp xếp: 3 chữ số đầu giống nhau -> 2 chữ số cuối giống với Telecom -> còn lại
    return {
        //Các nhà khai thác viễn thông Đài Loan
        '466-11': '中華電信', '466-92': '中華電信',
        '466-01': '遠傳電信', '466-03': '遠傳電信',
        '466-97': '台灣大哥大', '466-89': '台灣之星', '466-05': 'GT',
        //Các nhà khai thác viễn thông Trung Quốc
        ...
        //Các nhà khai thác viễn thông Hồng Kông
        ...
        //Các công ty viễn thông Nhật Bản
...
function getNetworkInfo(retryTimes = 5, retryInterval = 1000) {
    ...
    httpMethod.get('http://ip-api.com/json').then(response => {
        ...
        // Xác định xem có cơ hội thử lại không
        if (retryTimes > 0) {
            logger.error(error);
            ...
            $done({
                title: 'Đã xảy ra lỗi',
                content: 'Không thể lấy thông tin mạng hiện tại\nVui lòng kiểm tra trạng thái mạng và thử lại',
...
/**
* Logic chính, bắt đầu chương trình
*/
(() => {
    const retryTimes = 5;
    const retryInterval = 1000;
    // Surge Thời gian chờ của tập lệnh được đặt thành 30s
    // Kết thúc quá trình sớm 500ms
    const surgeMaxTimeout = 29500;
    // Hết thời gian chờ tập lệnh
    // retryTimes * 5000 Hết thời gian chờ cho mỗi yêu cầu mạng（Surge Thời gian chờ yêu cầu mạng là 5s）
