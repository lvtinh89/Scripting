...
function loadCarrierNames() {
  //Logic sắp xếp: 3 mã đầu giống nhau - 2 mã cuối cùng toán tử - còn lại
  return {
    //台湾运营商 Taiwan
...
function getIP() {
  ...
  if (!v4 && !v6) {
    ...
    if (v6?.primaryAddress) info.push(`Địa chỉ IPv6：Được bảo mật`);
    if (v4?.primaryRouter && getSSID()) info.push(`Bộ định tuyến IP：${v4?.primaryRouter}`);
    if (v6?.primaryRouter && getSSID()) info.push(`Địa chỉ IPv6：Được bảo mật`);
  }
  info = info.join("\n");
  return info + "\n";
}
...
function getNetworkInfo(retryTimes = 5, retryInterval = 1000) {
  ...
  httpMethod.get('http://ip-api.com/json').then(response => {
    ...
    if (retryTimes > 0) {
      ...
      // retryInterval Thực hiện lại chức năng sau một thời gian
...
/**
 * Logic chính, nhập chương trình
 */
(() => {
  const retryTimes = 5;
  const retryInterval = 1000;
  // Surge Thời gian chờ tập lệnh được đặt thành 30s
  // Trước 500ms Kết thúc quá trình theo cách thủ công
  const surgeMaxTimeout = 29500;
  // thời gian chờ tập lệnh
  // retryTimes * 5000 Đối với mỗi thời gian chờ yêu cầu mạng （Surge Thời gian chờ yêu cầu mạng mới là 5s）
  const scriptTimeout = retryTimes * 5000 + retryTimes * retryInterval;
  setTimeout(() => {
    logger.log("Script timeout");
    $done({
      title: "Yêu cầu đã hết thời gian chờ",
      content: "Yêu cầu kết nối đã quá hạn\n Vui lòng kiểm tra trạng thái mạng và thử lại",
      icon: 'wifi.exclamationmark',
      'icon-color': '#CB1B45',
    });
